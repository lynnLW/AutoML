...
library(survivalROC)
library(survival)
enet_cox <- function(train_data,fold,seed=123456){
  ##
  message("---1-1 Enet cox ---")
  set.seed(seed)
  ...
  for (alpha in seq(0.1,0.9,0.1)) {
    print("alpha")
    print(alpha)
    fit = cv.glmnet(as.matrix(x.train[,3:ncol(x.train)]),as.matrix(x.train[,1:2]) ,family = "cox",alpha=alpha,nfolds = 5)
  ...
  result$cindex = 1-rcorr.cens(pred_cox,Surv(x.test$time, x.test$status))[[1]]
  ### auc km ###
  cutoff = 12*1
  t.test=x.test$time
