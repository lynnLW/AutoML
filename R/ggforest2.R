...
ggforest2 <- function(model, data = NULL,
                     main = "Hazard ratio", cpositions=c(0.02, 0.1, 0.35),
  ...
  p <- ggplot(toShowExpClean, aes(seq_along(var), exp(estimate))) +
    geom_rect(aes(xmin = seq_along(var) - .5, xmax = seq_along(var) + .5,
                  ...
                  fill = ordered(seq_along(var) %% 2 + 1)),alpha = 0.15) +
    scale_fill_manual(values = c("white", "#0072b2"), guide = "none") +
    geom_point(pch = 16, size = 3.5,color="#0072b2") +
    geom_errorbar(aes(ymin = exp(conf.low), ymax = exp(conf.high)), width = 0.1,color="#0072b2") +
